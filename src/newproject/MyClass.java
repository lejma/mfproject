...
public class MyClass {
	...
	*/
    ...
    public static void testVerifyCorrectItemWasInBasket() {
		...
		4. click KOUPIT button - there should be a confirmation page with "zboûÌ bylo p¯id·no do koöÌku" hyperlink
	...
	*/
    ...
    public static void potential5thTest() {
    	/* verify that if users selects "Kur˝r po Praze" as a type of shipping the order cannot be finished when 
		...
		2. on Doprava a platba screen, select "Kur˝r po Praze" as a type of shipping - this should be accepted at this point
		3. proceed further to DodacÌ ˙daje screen, fill in any valid data in required fields, plus valid address in Prague
			3a. this should be accepted and order can be placed when DokonËit objedn·vku button is used
		4. repeat until point 3, but attempt to use address not residing in Prague - address should be accepted by the UI at this point
			4a. when DokonËit objedn·vku is pressed, backend validation should reject the order and inform user by a message
			4b. verify the message clearly highlights the issue, text should be "Objedn·vku nelze dokonËit, jelikoû zvolen˝ typ dopravy
			"Kur˝r po Praze" nenÌ podporov·n pro Vaöi adresu, prosÌm zvolte jin˝."
	*/
    }
